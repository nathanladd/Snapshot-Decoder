...
from infrastructure import get_logger, log_chart_generated, log_info, log_error, log_debug
...
class ChartController:
    ...
    def __init__(self):
        ...
        log_info("ChartController initialized")
    ...
    def render(self, config: ChartConfig, figure: Figure) -> Figure:
        ...
        """
        try:
            log_debug(f"Rendering chart: {config.chart_type}")
            ...
            log_info(f"Chart rendered successfully: {config.chart_type}")
            return rendered_figure
            
        except Exception as e:
            log_error(f"Chart rendering failed: {str(e)}")
            raise
    ...
    def render_thumbnail(self, config: ChartConfig, size: tuple = (200, 150)) -> Figure:
        ...
        """
        try:
            log_debug(f"Rendering thumbnail: {config.chart_type}")
            ...
            log_debug(f"Thumbnail rendered: {config.chart_type}")
            return thumbnail_figure
            
        except Exception as e:
            log_error(f"Thumbnail rendering failed: {str(e)}")
    ...
    def validate_config(self, config: ChartConfig) -> bool:
        ...
        try:
            ...
            if not hasattr(config, 'chart_type') or not config.chart_type:
                log_error("Chart config missing chart_type")
                return False
            
            if config.chart_type not in self.get_supported_chart_types():
                log_error(f"Unsupported chart type: {config.chart_type}")
                return False
            
            # Check data availability
            if not hasattr(config, 'snapshot') or not config.snapshot:
                log_error("Chart config missing snapshot data")
                return False
            ...
            if config.chart_type in ['line', 'scatter']:
                if not hasattr(config, 'primary_pids') or not config.primary_pids:
                    log_error(f"{config.chart_type} chart requires primary_pids")
                    return False
            
            elif config.chart_type == 'bar':
                if not hasattr(config, 'primary_pids') or not config.primary_pids:
                    log_error("Bar chart requires primary_pids")
                    return False
            
            elif config.chart_type == 'bubble':
                if not hasattr(config, 'primary_pids') or len(config.primary_pids) < 2:
                    log_error("Bubble chart requires at least 2 primary_pids")
                    return False
            
            log_debug(f"Chart config validation passed: {config.chart_type}")
            return True
            
        except Exception as e:
            log_error(f"Chart config validation failed: {str(e)}")
            return False
    ...
    def clear_figure(self, figure: Figure):
        ...
        try:
            figure.clear()
            plt.close(figure)
            log_debug("Figure cleared")
        except Exception as e:
            log_error(f"Failed to clear figure: {str(e)}")
    
    def shutdown(self):
        """Clean shutdown of chart controller."""
        log_info("ChartController shutdown complete")
