"""Update-available dialog: shows release notes + Update Now / Later buttons.
...
class _DownloadThread(QThread):
    ...
    finished = Signal(str)       # dest path, or "" if cancelled
    error = Signal(str)
...
class UpdateDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        header = QLabel(
            f"<b>Snapshot Decoder {self.latest_version} is available</b>"
            f"  <span style='color:#888;font-weight:normal;'>(you have v{APP_VERSION})</span>"
        )
    ...
    def _on_update_now(self) -> None:
        ...
        filename = self.download_url.split("/")[-1] or f"SnapshotDecoder_Setup-{self.latest_version}.exe"
